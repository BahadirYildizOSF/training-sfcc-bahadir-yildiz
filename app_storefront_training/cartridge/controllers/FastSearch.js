...
function HandleForm() {
    ...
    try {
        ...
        if(product === null){
            throw ({status: "PRODUCT_NOT_FOUND", statusText: Resource.msgf("product.notFound", "training", null, params.pid.stringValue)});
        } else if (product.master){
            throw ({status: "PRODUCT_IS_MASTER", statusText: Resource.msgf("product.isMaster", "training", null, product.getName())});
        }
        var availability = ProductModel.get(product).getAvailability(1);
        if(!availability.inStock){
            throw ({status: "PRODUCT_NOT_IN_STOCK", statusText: Resource.msgf("product.notInStock", "training", null, product.getName())});
        }
        var Cart = app.getModel('Cart').goc().addProductToCart();
